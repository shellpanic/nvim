...
ensure_tree_sitter_cli_void() {
   if is_cmd tree-sitter; then
      same "tree-sitter CLI already available"
      return 0
   fi
   ...
   if ensure_pkg_xbps tree-sitter 2>/dev/null; then
      is_cmd tree-sitter && {
         ok "tree-sitter CLI installed (tree-sitter)"
         return 0
      }
   fi
   ...
   if is_cmd cargo; then
      ...
      is_cmd tree-sitter && {
         ok "tree-sitter CLI available (cargo)"
         return 0
      }
   fi
   # No npm fallback to avoid node version/installer issues
   is_cmd tree-sitter && {
      ok "tree-sitter CLI available"
      return 0
   }
...
ensure_tree_sitter_cli_ubuntu() {
   ...
   is_cmd tree-sitter && {
      ok "tree-sitter CLI installed via apt"
      return 0
   }
   sudo apt-get install -y tree-sitter || true
   is_cmd tree-sitter && {
      ok "tree-sitter CLI available (tree-sitter)"
      return 0
   }
...
ensure_node_via_nvm() {
   ...
   if [ -s "$NVM_DIR/nvm.sh" ]; then
      ...
      if [ -n "${node_bin:-}" ] && [ -n "${npm_bin:-}" ]; then
         ...
         is_cmd node && is_cmd npm && {
            ok "Node $(node -v) and npm available (nvm)"
            return 0
         }
      fi
   fi
...
bootstrap_neovim_plugins() {
   ...
   note "Bootstrapping Neovim plugins (Lazy sync)"
